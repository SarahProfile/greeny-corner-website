...
export default function LoginPage() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('❌ Firebase login failed, trying direct API login:', err);

      // Firebase failed — try direct Laravel API login (for mobile app users)
      try {
        ...
        if (apiResponse.ok && apiData.access_token) {
          ...
          delete cleanUserData.access_token;
          delete cleanUserData.token;
          setAuthenticatedUser(cleanUserData, apiData.access_token);
          router.push('/my-plants');
          return;
        }

        // Both Firebase and API failed — show appropriate error
        if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
          setError(apiData.message || t('auth.noAccountFound'));
          ...
          setError(apiData.message || err.message || t('auth.backendAuthFailed'));
        }
      } catch (apiErr: any) {
        if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
      ...
      <div className="max-w-md w-full mx-4 animate-fadeIn">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-emerald-100 animate-slideUp" style={{animationDelay: '0.1s'}}>
          ...
          <div className="px-8 pb-8 border-t border-gray-100 pt-6 text-center">
            <p className="text-xs text-gray-500 mb-3">{t('common.getFullExperience')}</p>
